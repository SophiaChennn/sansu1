...
class Multi_Trainer(object):
    ...
    def __init__(self,model,data1,data2):
    ...
    def tower_loss(self, images, labels):

        #with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables):

        logit = self.model.h2d(images)

        losses = self.model.compute_loss(logit,labels)
    ...
    def train(self):
        ...
        with tf .Graph().as_default(), tf.device('/cpu:0'):
            ...
            images_placeholder = tf.placeholder(tf.float32,(cfg.batch_size*self.num_gpus,self.clip_len,cfg.image_size,cfg.image_size,3))
            labels_placeholder = tf.placeholder(tf.float32,(cfg.batch_size*self.num_gpus,2))
            ...
            weight_file = 'nets/inception_v2.ckpt'
            #weight_file = 'checkpoint2/model.ckpt-10000'
            ...
            with tf.variable_scope(tf.get_variable_scope()):
                ...
                for gpu_index in range(0,self.num_gpus):
                    ...
                    with tf.device('/gpu:%d' % gpu_index):
                        ...
                        loss,logit = self.tower_loss(images_placeholder[gpu_index * self.batch_size:(gpu_index + 1) * self.batch_size,:,:,:,:],
                                                     labels_placeholder[gpu_index * self.batch_size:(gpu_index + 1) * self.batch_size,:])
                        tf.get_variable_scope().reuse_variables()
                        batchnorm_updates = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
                        grads1 = opt.compute_gradients(loss)
                        #clipped_grads_and_vars = [(tf.clip_by_norm(grad, 10), var) for grad, var in grads1]
                        tower_grads.append(grads1)
                        logits.append(logit)

            self.exclude = ['change','conv3_x','conv4_x','conv5_x','fc_layer']
            self.variables_to_restore = slim.get_variables_to_restore(exclude = self.exclude)
            saver1 = tf.train.Saver(self.variables_to_restore)

            logits = tf.concat(logits,0)
            accuracy = self.tower_acc(logits,labels_placeholder)
            ...
            if weight_file is not None:
                ...
                saver1.restore(sess,weight_file)
            ...
            for step in range(cfg.max_iter + 5):
                ...
                if step % 5000 == 0 :
                    ...
                    for i in range(60):
                        ...
                        images,labels = self.data2.get()
                        feed_dict = {images_placeholder:images,
                                     labels_placeholder:labels}
                        acc = sess.run(accuracy,feed_dict = feed_dict)
                        acc_value = acc_value + acc
                    acc_value = acc_value / 60.0
...
def main():
    ...
    data2 = ucf_data('test')
    Trainer = Multi_Trainer(net,data1,data2)
    Trainer.train()
